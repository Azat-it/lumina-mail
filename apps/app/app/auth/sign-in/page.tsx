"use client"

import { signIn } from "@/lib/auth-client"
import { Badge } from "@workspace/ui/components/badge"
import { Button } from "@workspace/ui/components/button"
import { Card, CardContent, CardDescription, CardFooter, CardHeader, CardTitle } from "@workspace/ui/components/card"
import { Logo } from "@workspace/ui/components/logo"
import { cn } from "@workspace/ui/lib/utils"
import { Github, Mail, Users, BarChart, Sparkles, Zap } from "lucide-react"
import Image from "next/image"
export default function SignInPage() {
  return (
    <div className="min-h-screen flex">
      {/* Left side - Sign in form */}
      <div className="flex-1 flex flex-col justify-center px-4 sm:px-6 lg:px-8 bg-background">
        <div className="sm:mx-auto sm:w-full sm:max-w-md">
          <div className="flex justify-center">
            <Logo className="h-12 w-auto" />
          </div>
          <h2 className="mt-6 text-center text-2xl font-bold tracking-tight">
            Welcome to Lumina Mail
          </h2>
          <p className="mt-2 text-center text-sm text-muted-foreground">
            Sign in to your account to manage your email campaigns
          </p>
        </div>
        <SignInForm className="mt-8 sm:mx-auto sm:w-full sm:max-w-md" />
      </div>

      {/* Right side - Image and features */}
      <div className="hidden lg:block relative flex-1 bg-gradient-to-br from-white/5 via-white/10 to-black text-white">
        <div className="absolute inset-0">
          <Image src="/images/sign-in-background.jpg" alt="Sign in background" priority fill className="object-cover filter blur-sm brightness-50" />
          <div className="absolute inset-0 bg-black/10" />
          <div className="absolute inset-0 bg-gradient-to-t from-black via-transparent to-transparent" />
        </div>
        <div className="relative h-full flex items-center justify-center p-8">
          <div className="space-y-8 max-w-lg">
            <div className="space-y-4">
              <Badge>
                <Sparkles className="mr-2 h-4 w-4" />
                Simple yet powerful
              </Badge>
              <h2 className="text-4xl font-bold tracking-tight text-gray-500">
                Create beautiful email campaigns
                <span className="text-white ml-2">effortlessly</span>
              </h2>
              <p className="text-lg text-gray-500">
                Everything you need to create, send, and analyze your email campaigns in one place.
              </p>
            </div>

            <div className="grid gap-6 grid-cols-2">
              <div className="space-y-3">
                <div className="inline-flex items-center justify-center w-10 h-10 rounded-lg bg-white/10">
                  <Mail className="h-5 w-5 text-white" />
                </div>
                <h3 className="font-semibold">Email Editor</h3>
                <p className="text-sm text-gray-500">
                  Intuitive drag-and-drop editor with beautiful templates
                </p>
              </div>
              <div className="space-y-3">
                <div className="inline-flex items-center justify-center w-10 h-10 rounded-lg bg-white/10">
                  <Users className="h-5 w-5 text-white" />
                </div>
                <h3 className="font-semibold">Contact Management</h3>
                <p className="text-sm text-gray-500">
                  Organize and segment your contacts easily
                </p>
              </div>
              <div className="space-y-3">
                <div className="inline-flex items-center justify-center w-10 h-10 rounded-lg bg-white/10">
                  <BarChart className="h-5 w-5 text-white" />
                </div>
                <h3 className="font-semibold">Analytics</h3>
                <p className="text-sm text-gray-500">
                  Track performance with detailed insights
                </p>
              </div>
              <div className="space-y-3">
                <div className="inline-flex items-center justify-center w-10 h-10 rounded-lg bg-white/10">
                  <Zap className="h-5 w-5 text-white" />
                </div>
                <h3 className="font-semibold">Fast Delivery</h3>
                <p className="text-sm text-gray-500">
                  Reliable and quick email delivery
                </p>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  )
}

const SignInForm = ({ className }: { className?: string }) => {
  const signInWithGoogle = async () => {
    const data = await signIn.social({
      provider: "google"
    })
    console.log(data)
  }

  return (
    <Card className={cn("space-y-6", className)}>
      <CardHeader>
        <CardTitle>Sign in options</CardTitle>
      </CardHeader>
      <CardContent className="space-y-4">
        <Button
          className="w-full flex items-center justify-center gap-2"
          variant="outline"
          onClick={signInWithGoogle}
        >
          <svg className="h-5 w-5" viewBox="0 0 24 24">
            <path
              d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
              fill="#4285F4"
            />
            <path
              d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
              fill="#34A853"
            />
            <path
              d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
              fill="#FBBC05"
            />
            <path
              d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
              fill="#EA4335"
            />
          </svg>
          Sign in with Google
        </Button>
        <Button className="w-full flex items-center justify-center gap-2" variant="outline" disabled>
          <Github />
          Sign in with Github
        </Button>
      </CardContent>
      <CardFooter>
        <CardDescription>
          More options coming soon
        </CardDescription>
      </CardFooter>
    </Card>
  )
}

